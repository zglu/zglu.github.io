...
const easterEggs={"salary":"💰 <b>PROTOCOL 10.2.1:</b> Compensation details are encrypted. Let's discuss this during a <a href='mailto:luzg2008@gmail.com'>Direct Sync</a>.","coffee":"☕ <b>SYSTEM STATUS:</b> Caffeine levels nominal. Ready for a virtual coffee chat? Type <b>'contact'</b>.","life":"🧬 <b>42:</b> The answer to life, the universe, and everything... but in this terminal, the answer is usually 'High-quality NGS pipelines'.","sudo":"🔒 <b>PERMISSION DENIED:</b> You are not in the sudoers file. This incident will be reported (to my fish).","git":"💾 <b>GIT STATUS:</b> 1 file changed, 1,402 insertions(+), 1,200 deletions(-). <br>Status: <i>'Fixed one bug, created three more. Push anyway?'</i>","p-value":"📊 <b>STATISTICAL ALERT:</b> p = 0.049. <br>Status: Significant enough to publish, questionable enough to keep you up at night.","bug":"🐞 <b>DEBUG MODE:</b> It's not a bug, it's an undocumented biological feature. Please submit a Jira ticket (and then ignore it).","manual":"🛠️ <b>ERROR:</b> Manual labor detected. <br>My philosophy dictates that if you have to type 'manual', you should have written a script to do it for you.","lazy":"🚀 <b>PHILOSOPHY UPDATE:</b> I'm not lazy, I'm just highly motivated to <b>automate</b> myself out of a job so I can focus on the next big discovery."};function sendMessage(){const input=document.getElementById('user-input');const text=input.value.trim().toLowerCase();if(!text)return;addMessage(input.value,'user-msg');input.value='';let matches=ZGLU_DATA.knowledge.filter(k=>{return k.keywords.some(kw=>{const lowKw=kw.toLowerCase();const isExact=(lowKw===text);const isShorthand=lowKw.startsWith(text)&&text.length>=3;const isStandalone=new RegExp(`\\b${lowKw}\\b`,'i').test(text);return isExact||isShorthand||isStandalone})});if(matches.length>0){matches.sort((a,b)=>b.from-a.from);processMatches(matches)}else{if(easterEggs[text]){setTimeout(()=>addMessage(easterEggs[text],'bot-msg'),300);return}
let bestMatch=null;let minDistance=3;ZGLU_DATA.knowledge.forEach(k=>{k.keywords.forEach(kw=>{const dist=getLevenshteinDistance(text,kw);if(dist<minDistance){minDistance=dist;bestMatch=kw}})});setTimeout(()=>{if(bestMatch){addMessage(`No direct hit for "${text}". Did you mean <b style="cursor:pointer; color:var(--accent-color)" onclick="quickAsk('${bestMatch}')">${bestMatch}</b>?`,'bot-msg')}else{addMessage("🚫 Query not recognized. Try 'help' to see available tracks.",'bot-msg')}},350)}}
...
function generateContactCard(){const info=ZGLU_DATA.contact_info;return `
    <div style="background: rgba(56, 189, 248, 0.05); border: 1px solid var(--accent-color); border-radius: 12px; padding: 20px; margin-top: 10px; text-align: center;">
        ...
        <div style="text-align: left; font-size: 13px; margin-bottom: 20px; display: inline-block;">
            <div style="margin: 5px 0;">📍 ${info.location}</div>
            <div style="margin: 5px 0;">🌐 ${info.web}</div>
        </div>
